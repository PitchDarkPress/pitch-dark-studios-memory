...
<script>
  ...
  function snapAll(){
    ...
    repositionPinned();
    // the tracks and scenes have only just been built/grown; let the layout settle,
    // then place the cards once more against the now-correct heights so none ride over a title.
    requestAnimationFrame(function(){ repositionPinned(); });
  }
  ...
  function headerHeightFor(si){
    ...
    // measure the title block (the scene title + its underline) and leave a clear gap below it,
    // so a card always lands beneath the title rather than riding up over it.
    var title = sb.querySelector('.sc-title'), uline = sb.querySelector('.sc-uline');
    var titleH = title ? title.offsetHeight : 34;
    var ulineH = uline ? uline.offsetHeight : 1;
    var ulineGap = 14;                          // the underline's own top+bottom margins
    var topPad = 6, gapBelow = 14;              // band padding above + breathing room below the title
    return Math.max(76, topPad + titleH + ulineGap + ulineH + gapBelow);
  }
  ...
  function placePinnedCards(){
    ...
    // if cards were pinned while there were no scenes, and a scene now exists,
    // adopt the first scene as their home so they sit below its title.
    if (bands.length){
      var rehomed = false;
      CARDS.forEach(function(c){ if (c.pinned && (c.homeScene == null || c.homeScene < 0)){ c.homeScene = 0; rehomed = true; } });
      if (rehomed) map = cardsByCell();
    }
